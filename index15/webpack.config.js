...
module.exports = {
  ...
  devServer: {
    ...
    port: 8090,
    hot: true, // webpack 开启 hot module replacement 热模块更新
    hotOnly: true // 即使 html 的功能不生效，就不让页面自动刷新
  },
  ...
  module: {
    rules: [{
      ...
      test: /\.js$/,
      exclude: /node_modules/, // 排除掉 node_modules (第三方模块)
      loader: 'babel-loader', // 只是连接了 babel 和 webpack，并不会转化 es6 的语法
      options: {
        presets: [
          ['@babel/preset-env', {
            targets: {
              chrome: '67' // 项目运行在 >67 版本的 Chrome 浏览器下
            },
            useBuiltIns: 'usage' // 按需引入 @babel/polyfill，如果不配置这个，只在入口文件引入 @babel/polyfill 的话，就是全部引入，会导致打包生成的文件大很多
          }]
        ]
      }
    }]
  ...
  plugins: [
    ...
    new webpack.HotModuleReplacementPlugin() // 开启 HMR 功能，也需要这个插件
  ],
  output: { // 打包出的文件配置
